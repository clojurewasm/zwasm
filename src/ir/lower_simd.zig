...
pub fn emitPrefixFD(self: *Lowerer) Error!void {
    ...
    switch (sub) {
        ...
        // §15.4 / D-246 — saturating add/sub + Q15 mulr + extadd_pairwise.
        // Per Wasm SIMD spec (BinarySIMD.md / UnarySIMD.md): the lower-side
        // sub-op→ZirOp wiring was missing, so these were interp-only on both
        // arches. arm64 emit = SQADD/UQADD/SQSUB/UQSUB (.16B/.8H), SQRDMULH.8H,
        // SADDLP/UADDLP (add-long-pairwise); x86_64 emit already present.
        111 => try self.emit(.@"i8x16.add_sat_s", 0, 0),
        112 => try self.emit(.@"i8x16.add_sat_u", 0, 0),
        113 => try self.emit(.@"i8x16.sub", 0, 0),
        114 => try self.emit(.@"i8x16.sub_sat_s", 0, 0),
        115 => try self.emit(.@"i8x16.sub_sat_u", 0, 0),
        124 => try self.emit(.@"i16x8.extadd_pairwise_i8x16_s", 0, 0),
        125 => try self.emit(.@"i16x8.extadd_pairwise_i8x16_u", 0, 0),
        126 => try self.emit(.@"i32x4.extadd_pairwise_i16x8_s", 0, 0),
        127 => try self.emit(.@"i32x4.extadd_pairwise_i16x8_u", 0, 0),
        128 => try self.emit(.@"i16x8.abs", 0, 0),
        129 => try self.emit(.@"i16x8.neg", 0, 0),
        130 => try self.emit(.@"i16x8.q15mulr_sat_s", 0, 0),
        142 => try self.emit(.@"i16x8.add", 0, 0),
        143 => try self.emit(.@"i16x8.add_sat_s", 0, 0),
        144 => try self.emit(.@"i16x8.add_sat_u", 0, 0),
        145 => try self.emit(.@"i16x8.sub", 0, 0),
        146 => try self.emit(.@"i16x8.sub_sat_s", 0, 0),
        147 => try self.emit(.@"i16x8.sub_sat_u", 0, 0),
