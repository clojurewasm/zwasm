//! ZIR opcode catalog — extracted from `zir.zig` per ADR-0087.
//!
//! Pure tag enum (`enum(u16)`) with no methods on the type;
//! lives in its own file because of size (684 LOC of variants).
//! `zir.zig` re-exports `ZirOp` so external callers reach
//! `zir.ZirOp` identically.
//!
