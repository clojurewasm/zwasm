...
// ADR-0207 M3 transitional: dissolves when the classifier + init-install move
// to the facade at M3-b (final graph: (a) imports no sibling).
...
pub const WasiP2Ctx = struct {
    ...
    pub fn init(alloc: Allocator, host: *wasi_host.Host) !WasiP2Ctx {
        return .{
            .alloc = alloc,
            .host = host,
            // M3 transitional (ADR-0207): init still installs the hooks; the
            // install moves to the creation sites when the classifier and
            // orchestration leave for the facade at M3-b.
            .p3_hooks = p3h.hooksTable(),
