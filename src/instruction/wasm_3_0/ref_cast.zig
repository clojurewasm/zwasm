//! `ref.cast` — Wasm 3.0 GC proposal (ref/cast + i31 cohort).
//!
...
pub const op_tag: ZirOp = .@"ref.cast";
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_ref_cast,
    .lower = lower_ref_cast,
    .interp = interp_ref_cast,
};

fn validate_ref_cast() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_ref_cast() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_ref_cast() collector.DispatchError!void {
    return error.NotMigrated;
}
