//! `array.len` — Wasm 3.0 GC proposal (array cohort).
//!
...
pub const op_tag: ZirOp = .@"array.len";
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_array_len,
    .lower = lower_array_len,
    .interp = interp_array_len,
};

fn validate_array_len() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_array_len() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_array_len() collector.DispatchError!void {
    return error.NotMigrated;
}
