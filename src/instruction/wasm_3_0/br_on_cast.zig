//! `br_on_cast` — Wasm 3.0 GC proposal (ref/cast + i31 cohort).
//!
...
pub const op_tag: ZirOp = .br_on_cast;
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_br_on_cast,
    .lower = lower_br_on_cast,
    .interp = interp_br_on_cast,
};

fn validate_br_on_cast() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_br_on_cast() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_br_on_cast() collector.DispatchError!void {
    return error.NotMigrated;
}
