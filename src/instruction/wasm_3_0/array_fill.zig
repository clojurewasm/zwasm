//! `array.fill` — Wasm 3.0 GC proposal (array cohort).
//!
...
pub const op_tag: ZirOp = .@"array.fill";
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_array_fill,
    .lower = lower_array_fill,
    .interp = interp_array_fill,
};

fn validate_array_fill() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_array_fill() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_array_fill() collector.DispatchError!void {
    return error.NotMigrated;
}
