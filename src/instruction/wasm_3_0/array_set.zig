//! `array.set` — Wasm 3.0 GC proposal (array cohort).
//!
...
pub const op_tag: ZirOp = .@"array.set";
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_array_set,
    .lower = lower_array_set,
    .interp = interp_array_set,
};

fn validate_array_set() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_array_set() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_array_set() collector.DispatchError!void {
    return error.NotMigrated;
}
