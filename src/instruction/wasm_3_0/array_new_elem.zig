//! `array.new_elem` — Wasm 3.0 GC proposal (array cohort).
//!
...
pub const op_tag: ZirOp = .@"array.new_elem";
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_array_new_elem,
    .lower = lower_array_new_elem,
    .interp = interp_array_new_elem,
};

fn validate_array_new_elem() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_array_new_elem() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_array_new_elem() collector.DispatchError!void {
    return error.NotMigrated;
}
