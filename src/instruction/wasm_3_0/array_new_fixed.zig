//! `array.new_fixed` — Wasm 3.0 GC proposal (array cohort).
//!
...
pub const op_tag: ZirOp = .@"array.new_fixed";
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_array_new_fixed,
    .lower = lower_array_new_fixed,
    .interp = interp_array_new_fixed,
};

fn validate_array_new_fixed() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_array_new_fixed() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_array_new_fixed() collector.DispatchError!void {
    return error.NotMigrated;
}
