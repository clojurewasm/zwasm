//! `array.copy` — Wasm 3.0 GC proposal (array cohort).
//!
...
pub const op_tag: ZirOp = .@"array.copy";
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_array_copy,
    .lower = lower_array_copy,
    .interp = interp_array_copy,
};

fn validate_array_copy() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_array_copy() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_array_copy() collector.DispatchError!void {
    return error.NotMigrated;
}
