//! `array.get` — Wasm 3.0 GC proposal (array cohort).
//!
...
pub const op_tag: ZirOp = .@"array.get";
pub const wasm_level: ?WasmLevel = .v3_0;
pub const wasi_level: ?WasiLevel = null;
pub const enable_features: []const Feature = &.{};

pub const handlers = .{
    .validate = validate_array_get,
    .lower = lower_array_get,
    .interp = interp_array_get,
};

fn validate_array_get() collector.DispatchError!void {
    return error.NotMigrated;
}

fn lower_array_get() collector.DispatchError!void {
    return error.NotMigrated;
}

fn interp_array_get() collector.DispatchError!void {
    return error.NotMigrated;
}
