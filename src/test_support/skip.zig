...
pub const Blocker = enum {
    ...
    @"D-212",
    /// D-314 #3a — x86_64 JIT prologue interrupt poll. arm64 landed first;
    /// the x86_64 poll (after the JBE stack probe) is the next chunk. The
    /// interrupt-trap test arch-gates to aarch64 until then.
    @"D-314",
};
