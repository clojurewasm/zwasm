...
//!
//! Phase-7 / 7.2 chunk-b scope: declarative tables only +
//! the `slotToReg` mapper. Real spill-slot allocation lives in
//! §9.7 / 7.1's regalloc; this module just translates "slot N
...
/// First 8 X-registers carry function arguments (and return
/// values; x0 is the primary return register).
pub const arg_gprs = [_]Xn{ 0, 1, 2, 3, 4, 5, 6, 7 };

/// X8 carries the indirect-result-location pointer when the
/// callee returns a struct via memory.
...
/// for the OS; we treat it as never-allocatable here.
...
/// Pool of GPR registers the regalloc may freely use, in
/// priority order: caller-saved scratch first (cheapest, no
/// prologue cost), then callee-saved (forces save/restore).
/// X0..X7 are NOT in this pool by default — emit reserves them
/// for arg / return marshalling. X8 / X16-18 / X29-30 are
...
/// Is `xn` caller-saved per AAPCS64? Used by the emit pass to
/// decide whether a vreg held in `xn` needs a save before a
/// call.
...
/// for the allocatable range; FP/LR/SP/PR are special and
/// return false.
...
test "slotToReg: slot 0 → x9 (first caller-scratch)" {
    try testing.expectEqual(@as(Xn, 9), slotToReg(0).?);
}

test "slotToReg: slot 7 → x16 — wait, slot 7 should be x16 (last caller-scratch is x15)" {
    // caller_saved_scratch_gprs = {9..15} (7 regs). Slot 7 falls
    // into callee_saved_gprs[0] = x19.
...
test "isCallerSaved: x0..x7, x9..x17 are caller-saved" {
...
test "frame_pointer and link_register tracked correctly" {
