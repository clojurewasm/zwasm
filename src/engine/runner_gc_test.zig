...
test "runI32Export: struct.new + struct.get of a v128 field + extract_lane 3 → 44 (D-460 JIT v128-GC)" {
    ...
    // The v128 is built via splat+replace_lane (NOT v128.const, which has a
    // separate JIT-operand limit) so lane 3 (bytes [12,16) of the field slot)
    // holds 44 ≠ the other lanes' 1. An 8-byte-truncated (GPR) load/store would
    // drop lane 3 → 0/garbage, so this pins the full 16-byte v128 field slot.
    const bytes = [_]u8{
        0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x09, 0x02, 0x5f, 0x01, 0x7b, 0x01, 0x60,
        0x00, 0x01, 0x7f, 0x03, 0x02, 0x01, 0x01, 0x07, 0x05, 0x01, 0x01, 0x66, 0x00, 0x00, 0x0a, 0x17,
        0x01, 0x15, 0x00, 0x41, 0x01, 0xfd, 0x11, 0x41, 0x2c, 0xfd, 0x1c, 0x03, 0xfb, 0x00, 0x00, 0xfb,
        0x02, 0x00, 0x00, 0xfd, 0x1b, 0x03, 0x0b,
    };
...
test "runI32Export: struct.set of a v128 field then struct.get + extract_lane 3 → 44 (D-460 JIT v128-GC)" {
    ...
    // path of struct.set (default-init writes 0s, the set overwrites lane 3).
    const bytes = [_]u8{
        0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x09, 0x02, 0x5f, 0x01, 0x7b, 0x01, 0x60,
        0x00, 0x01, 0x7f, 0x03, 0x02, 0x01, 0x01, 0x07, 0x05, 0x01, 0x01, 0x66, 0x00, 0x00, 0x0a, 0x24,
        0x01, 0x22, 0x01, 0x01, 0x63, 0x00, 0xfb, 0x01, 0x00, 0x21, 0x00, 0x20, 0x00, 0x41, 0x01, 0xfd,
        0x11, 0x41, 0x2c, 0xfd, 0x1c, 0x03, 0xfb, 0x05, 0x00, 0x00, 0x20, 0x00, 0xfb, 0x02, 0x00, 0x00,
        0xfd, 0x1b, 0x03, 0x0b,
    };
    try testing.expectEqual(@as(u32, 44), runI32Export(testing.allocator, &bytes, "f"));
}
