...
pub fn compile(
    ...
    for (func.instrs.items, 0..) |ins, pc| {
        ...
        switch (ins.op) {
            ...
            .@"i8x16.eq" => try op_simd.emitI8x16Eq(&ctx, &ins),
            .@"i8x16.ne" => try op_simd.emitI8x16Ne(&ctx, &ins),
            .@"i8x16.lt_s" => try op_simd.emitI8x16LtS(&ctx, &ins),
            .@"i8x16.lt_u" => try op_simd.emitI8x16LtU(&ctx, &ins),
            .@"i8x16.gt_s" => try op_simd.emitI8x16GtS(&ctx, &ins),
            .@"i8x16.gt_u" => try op_simd.emitI8x16GtU(&ctx, &ins),
            .@"i8x16.le_s" => try op_simd.emitI8x16LeS(&ctx, &ins),
            .@"i8x16.le_u" => try op_simd.emitI8x16LeU(&ctx, &ins),
            .@"i8x16.ge_s" => try op_simd.emitI8x16GeS(&ctx, &ins),
            .@"i8x16.ge_u" => try op_simd.emitI8x16GeU(&ctx, &ins),
            .@"i16x8.eq" => try op_simd.emitI16x8Eq(&ctx, &ins),
            .@"i16x8.ne" => try op_simd.emitI16x8Ne(&ctx, &ins),
            .@"i16x8.lt_s" => try op_simd.emitI16x8LtS(&ctx, &ins),
            .@"i16x8.lt_u" => try op_simd.emitI16x8LtU(&ctx, &ins),
            .@"i16x8.gt_s" => try op_simd.emitI16x8GtS(&ctx, &ins),
            .@"i16x8.gt_u" => try op_simd.emitI16x8GtU(&ctx, &ins),
            .@"i16x8.le_s" => try op_simd.emitI16x8LeS(&ctx, &ins),
            .@"i16x8.le_u" => try op_simd.emitI16x8LeU(&ctx, &ins),
            .@"i16x8.ge_s" => try op_simd.emitI16x8GeS(&ctx, &ins),
            .@"i16x8.ge_u" => try op_simd.emitI16x8GeU(&ctx, &ins),
            .@"i32x4.eq" => try op_simd.emitI32x4Eq(&ctx, &ins),
            .@"i32x4.ne" => try op_simd.emitI32x4Ne(&ctx, &ins),
            .@"i32x4.lt_s" => try op_simd.emitI32x4LtS(&ctx, &ins),
            .@"i32x4.lt_u" => try op_simd.emitI32x4LtU(&ctx, &ins),
            .@"i32x4.gt_s" => try op_simd.emitI32x4GtS(&ctx, &ins),
            .@"i32x4.gt_u" => try op_simd.emitI32x4GtU(&ctx, &ins),
            .@"i32x4.le_s" => try op_simd.emitI32x4LeS(&ctx, &ins),
            .@"i32x4.le_u" => try op_simd.emitI32x4LeU(&ctx, &ins),
            .@"i32x4.ge_s" => try op_simd.emitI32x4GeS(&ctx, &ins),
            .@"i32x4.ge_u" => try op_simd.emitI32x4GeU(&ctx, &ins),
            .@"i64x2.eq" => try op_simd.emitI64x2Eq(&ctx, &ins),
            .@"i64x2.ne" => try op_simd.emitI64x2Ne(&ctx, &ins),
            .@"i64x2.lt_s" => try op_simd.emitI64x2LtS(&ctx, &ins),
