...
test "compile: empty body without liveness errors" {
    ...
    try testing.expectError(Error.AllocationMissing, compile(testing.allocator, &f, empty_alloc, &.{}, &.{}, 0, &.{}, &.{}));
}
...
test "compile: local.tee writes to local but keeps value pushed" {
    ...
    try testing.expectEqual(@as(usize, 72), out.bytes.len);
    const body0 = prologue.body_start_offset(true);
...
test "compile: (param i64) — prologue stores X1 to [SP, #0] (STR X width)" {
    ...
    try testing.expectEqual(expected_str_x, std.mem.readInt(u32, out.bytes[44..][0..4], .little));
}
...
test "compile: (param f32) — prologue stores S0 to [SP, #0] (STR S width)" {
    ...
    try testing.expectEqual(expected, std.mem.readInt(u32, out.bytes[44..][0..4], .little));
}
...
test "compile: mixed (param i32 f32 i64 f64) — independent X/V counters" {
    ...
    try testing.expectEqual(inst.encStrImmW(1, 31, 0), std.mem.readInt(u32, out.bytes[44..][0..4], .little));
    try testing.expectEqual(inst.encStrSImm(0, 31, 8), std.mem.readInt(u32, out.bytes[48..][0..4], .little));
    try testing.expectEqual(inst.encStrImm(2, 31, 16), std.mem.readInt(u32, out.bytes[52..][0..4], .little));
    try testing.expectEqual(inst.encStrDImm(1, 31, 24), std.mem.readInt(u32, out.bytes[56..][0..4], .little));
}
...
test "compile: (param i32) (result i32) local.get 0 — prologue stores W1 to [SP, #0]" {
    ...
    try testing.expectEqual(expected_str, std.mem.readInt(u32, out.bytes[44..][0..4], .little));
    // Body: LDR W into the param's vreg (slot 0 → W9 by AAPCS64
    // allocatable pool ordering).
    const expected_ldr = inst.encLdrImmW(9, 31, 0);
    try testing.expectEqual(expected_ldr, std.mem.readInt(u32, out.bytes[48..][0..4], .little));
}
...
test "compile: (param i32 i32) — prologue stores W1, W2 to [SP, #0], [SP, #8]" {
    ...
    try testing.expectEqual(inst.encStrImmW(1, 31, 0), std.mem.readInt(u32, out.bytes[44..][0..4], .little));
    try testing.expectEqual(inst.encStrImmW(2, 31, 8), std.mem.readInt(u32, out.bytes[48..][0..4], .little));
    // Body LDR for local.get 1 → reads from [SP, #8].
    try testing.expectEqual(inst.encLdrImmW(9, 31, 8), std.mem.readInt(u32, out.bytes[52..][0..4], .little));
}
