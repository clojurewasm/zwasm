//! arm64 emit handler for `array.new_data` — Wasm 3.0 GC §3.3.5.6.7.
//! Pop i32 offset + i32 size (size on top), allocate a `size`-element
//! array of type `ins.payload` and copy its payload from data segment
//! `ins.extra` at byte `offset` — done inside the `jitGcArrayNewData(rt,
//! typeidx, segidx, offset, size)` trampoline (returns the GcRef, or 0 on
//! trap), so the emit is a plain 5-arg marshal + CALL + trap branch on a 0
//! result + capture the ref. 2 → 1.
//!
//! typeidx + segidx are compile-time immediates (W1/W2 via MOVZ/MOVK);
//! offset + size are the popped operands (W3/W4). All consumed before the
//! BLR (strict `is_call`); the result ref is captured from W0 AFTER the
//! call. Arg regs X0..X4 are NOT in the regalloc allocatable pool, so the
//! marshal has no parallel-move hazard. The trampoline reuses the same
//! `data_segments_ptr` descriptors `memory.init` uses (ADR-0056 m-3b).
//!
//! Lowering: MOV W3 = offset; MOV W4 = size; MOV X0, X19 (rt); MOVZ/MOVK
//! W1 = typeidx; MOVZ/MOVK W2 = segidx; MOVZ/MOVK X16 = &jitGcArrayNewData;
...
const scratch: inst.Xn = 16; // IP0 — &jitGcArrayNewData.
