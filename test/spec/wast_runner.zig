...
pub fn main(init: std.process.Init) !void {
    ...
    try stdout.print("wast_runner: lines={d} accounted={d} residual={d} (residual = non-assertion directives + any line that reached no column)\n", .{ lines, passed + failed, if (lines > passed + failed) lines - (passed + failed) else 0 });
    try stdout.flush();
