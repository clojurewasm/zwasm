...
SKIP_PATTERNS="gc|array|struct|extern|tag|exception|memory64|address64|align64|binary_leb128_64|annotations|ref_|any_|i31|sub_|type_|table_copy_mixed|table_get_mixed|elem_mixed|br_on_|extern_|multi_memory|rec_|return_call|try_|throw_|rethrow_"
...
for wast in "$TESTSUITE"/*.wast; do
    ...
    if wast2json "$wast" -o "$OUTDIR/$name.json" 2>/dev/null; then
