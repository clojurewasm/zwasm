;; WASI Preview 2 component that calls wasi:random/random.get-random-bytes(16),
;; ORs the 16 returned bytes, and exit(0) iff len==16 AND some byte is nonzero
;; (i.e. real random was written), else exit(1). Exercises the random_get_bytes
;; trampoline (Phase D3-4): get-random-bytes(len: u64) -> list<u8> lowers to a
;; core (i64 len, i32 retptr); the trampoline allocates `len` bytes via the
;; guest cabi_realloc (nested invoke), fills with secure random, and writes
;; (data_ptr, len) to retptr. Verdict surfaces via the exit channel (D3-1).
(component
  ...
  (core func $rand
    (canon lower (func $random "get-random-bytes") (memory $libc "memory") (realloc $cabi_realloc)))
