...
| 2026-05-19 | `<backfill>` | scope     | **§4.5 amend (per-op file pattern 正式採用 — Phase 9 完備 substrate audit Q3 採用; per ADR-0071 + ADR-0073)**. §4.5 (DispatchTable feature modules) を以下に sharpen: (a) `DispatchTable.interp` 軸 = required (`mvp/mod.zig` で完成済; 他 features は §9.12-B で完成); (b) `validator` / `lower` / `arm64/emit` / `x86_64/emit` 軸 = per-op file pattern (`src/instruction/wasm_X_Y/<op>.zig` で `pub const handlers = .{ .validate, .lower, .arm64, .x86_64, .interp }` を export); (c) 中央 dispatcher は `src/ir/dispatch_collector.zig` (新設; §9.12-B で land) が comptime に全 op file を import + filter + `inline switch` dispatch を生成; (d) `build_options.wasm_level` / `wasi_level` で `comptime` filter (build-option DCE; per ADR-0073)。`src/feature/<feature>/register.zig` は interp 軸のみ register; 他 9 features の placeholder は §9.12-B で interp axis only と明文化。skeleton; 詳細は §9.12-pre / §9.12-B で。 |
