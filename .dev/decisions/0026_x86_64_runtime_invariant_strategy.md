...
| 2026-05-18 | `<backfill>` | **Amendment**: MEMORY-class return convention (ADR-0069 §Phase 2 chunk (b)-e-3). The original ADR-0026 reserved R15 for runtime_ptr and left RAX as the result-passing reg; this amendment specifies the hidden indirect-result-pointer reg for `sig.results.len > 2` returns. **Decision**: zwasm internal calls use **R11** to carry the hidden buffer pointer from caller to callee (SysV only; Win64 deferred to §9.13-0). RDI stays as runtime_ptr / entry_arg0 unchanged. The callee prologue captures R11 into an 8 B frame slot below the spill region; the epilogue loads it back into RAX (SysV §3.2.3 RAX=&buffer compliance bonus + outside `spill_stage_gprs` cohort to dodge `gprLoadSpilled` clobber) and writes each result to `[RAX + i*8]`. The caller emits `LEA R11, [RSP + per_call_overflow_bytes]` after `marshalCallArgs` (the LEA happens after the last R10/R11 spill-stage use) and before the `entry_arg0` restore. **Alternative considered + rejected**: shift to standard SysV §3.2.3 (RDI=buffer, RSI=runtime_ptr). Rejected because it requires `marshalCallArgs` to shift arg-passing regs by one slot for MEMORY-class callees AND splits the prologue into Class A/B/C variants. The R11 internal convention keeps the JIT's arg-passing reg map stable; the Zig→JIT entry helper (chunk (b)-e-4) absorbs the standard-SysV bridging via an inline-asm thunk (established Class B pattern). **References**: ADR-0017 2026-05-18 amend (arm64 X8 mirror), ADR-0069 §Phase 2 (multi-result return ABI), lesson `2026-05-18-class-c-callee-without-caller-segvs-fac.md` (bundling constraint). |
