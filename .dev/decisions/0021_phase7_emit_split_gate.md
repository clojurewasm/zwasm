...
| Date       | Commit       | Summary                            |
|------------|--------------|------------------------------------|
| 2026-05-04 | `4a2da3c3` | Initial Decision; sub-gate inserted. |
| 2026-05-04 | `04c2453f` | Sub-deliverable a scope reduced. **Why (gap, per the `adr-revision-history-misuse` lesson written this session)**: the original Decision text said "Migrate 142 relativisable test sites" in the same commit as the helper module. In practice that's a 132-site mechanical rewrite that benefits from running alongside the emit.zig split (sub-b), not as a standalone commit racing the rest of this session's design work. **What changed**: helper module `src/engine/codegen/arm64/prologue.zig` lands per the original plan; pattern demonstrated at 4 representative test sites + new rule (`edge_case_testing.md` §"Test-side byte offsets must be relative") gates new sites. Bulk migration of the remaining ~128 sites runs under sub-b. The hard gate "row 7.6 does not open until 7.5d closes" is unchanged — both sub-a and sub-b must close before x86_64 work begins. |
| 2026-05-05 | `bb404088` | Sub-deliverable b module count: actual 12, not "~9". **Why (refinement, per `adr-revision-history-misuse` categorisation)**: the original Decision text said "split into ~9 modules" with a representative list (emit.zig + ops_const + ops_alu + ops_memory + ops_control + ops_call + bounds_check + register + emit_helpers + label = 10 entries; "~9" was approximate). Two structural deviations forced a higher count during implementation. **What changed**: (1) `ops_alu` sub-split into `op_alu_int.zig` + `op_alu_float.zig` + `op_convert.zig` because a single ops_alu module would exceed the per-module 400-LOC cap (~1500 LOC if combined). The 400-LOC cap is the load-bearing constraint per Decision §"orchestrator ≤ 1000 LOC; each module ≤ 400 LOC"; the "≤ 9 modules" target is approximate and yields when the cap binds. (2) `register.zig` + `emit_helpers.zig` merged into a single `gpr.zig` (writeU32 + resolve helpers + spill staging — 124 LOC); the predicted split between "register table" and "emit helpers" turned out artificial because both consumers always co-occur. (3) `ctx.zig` added as a parameter-bundle for op-handler modules (EmitCtx + Error + CallFixup) — not in the original list but necessary to avoid every op_*.zig importing emit.zig (circular). (4) `emit_test.zig` extracted as chunk 10 (1947 LOC of inline tests) to bring emit.zig under the §A2 hard 2000-LOC cap; not in the original plan because the original count assumed orchestrator-LOC excluded tests, but `file_size_check.sh` doesn't make that distinction. **Final shape**: 12 module files (emit.zig orchestrator 580 LOC + 11 extracted modules ≤ 400 LOC each + emit_test.zig 1986 LOC). The "≤ 9 modules" guideline reads in retrospect as "the per-module 400-LOC cap takes precedence; module count emerges from it". Rule applied to ADR-0026's x86_64 layout decision (which reuses the same per-module shape). |
