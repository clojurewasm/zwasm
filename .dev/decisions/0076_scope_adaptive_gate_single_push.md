...
| 2026-07-03 | _(this)_ | D9 amend — RETIRE the local Windows-gating cadence (post-v2.0.0 maintenance, user-ratified via the scaffolding-necessity audit §B.2/§C). `main` is now PR-only and CI's `ci-required` runs the Windows leg on **every** PR, so `should_gate_windows.sh`'s BATCHED/`--suspend` heuristic is no longer load-bearing for merge safety → gutted to a deprecation stub. `gate_merge.sh` demoted from "the A13 merge gate" to an **optional** pre-PR pre-flight that mirrors CI (CI's `ci-required` 3-OS gate is now authoritative). Also **PROMOTE `zone_check` into `ci_gate.sh`** (Zone 0–3 layering is now a real CI merge gate, not a local-only nudge). `spill_aware_check` promotion is **HELD (D-505)**: it was never wired into `gate_commit`, so 7 pre-existing violations accumulated in arm64 SIMD handlers; promote only after triage. |
