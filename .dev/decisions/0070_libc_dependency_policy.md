...
| 2026-08-11 | (pending) | **ADR-0205 phase-C amendment** — add `posix.system.socket` / `posix.system.bind` / `posix.system.connect` / `posix.system.close` / `posix.system.listen` / `posix.system.getsockname` (two sites: `rawBoundConnect` and `posixListen`, both `src/wasi/p2_sockets.zig`) to Necessary. `rawBoundConnect` covers the WASI-0.3 `tcp.bind` → `connect` transition (official sockets-tcp-connect test_explicit_bind): connect-FROM-a-bound-socket, which the pinned `std.Io.net` cannot express (its connect always creates its own socket). `posixListen` composes a listen with **SO_REUSEADDR only** (no SO_REUSEPORT): the pinned stdlib's `reuse_address` couples SO_REUSEPORT on POSIX, and clearing it after bind is ineffective on Linux (the bind bucket caches `fastreuseport` at first bind), so a second bind on a live listener would wrongly succeed — breaking the WIT address-in-use contract (official sockets-tcp-bind). `posix.system.*` resolves to `std.c` on every zwasm build (`link_libc` always on) — spelled via `posix.system` rather than `std.c` but treated as boundary sites all the same (the `check_libc_boundary.sh` grep does not lexically match them; this row is the authoritative record). The composed fds plug into the normal `net.Stream` / `net.Server` vtable paths. POSIX-only sites: windows sockets under the pinned stdlib are NT/AFD handles with no libc surface — the windows side composes the equivalent flows from NT ioctls instead (`winOpenSocketAfd`/`winAfdBind`/`winAfdConnect`/`winAfdGetSockName`, same file; ntdll externs are off the libc boundary, same class as the kernel32 note above). Upstream watch: a `std.Io.net` bound-connect (`ConnectOptions.local_address`-class API) + a REUSEADDR-without-REUSEPORT listen option absorb all six posix sites. |
