...
| 2026-08-11 | (pending) | **ADR-0205 phase-C amendment** — add `posix.system.socket` / `posix.system.bind` / `posix.system.connect` / `posix.system.close` (one site: `rawBoundConnect`, `src/wasi/p2_sockets.zig`) to Necessary. The WASI-0.3 `tcp.bind` → `connect` transition (official sockets-tcp-connect test_explicit_bind) needs connect-FROM-a-bound-socket, which the pinned `std.Io.net` cannot express (its connect always creates its own socket). `posix.system.*` resolves to `std.c` on every zwasm build (`link_libc` always on) — spelled via `posix.system` rather than `std.c` but treated as boundary sites all the same (the `check_libc_boundary.sh` grep does not lexically match them; this row is the authoritative record). The composed fd plugs into the normal `net.Stream` vtable paths. POSIX-only: windows sockets under the pinned stdlib are NT/AFD handles with no libc surface — bound-connect there is truthful not-supported (D-569). Upstream watch: a `std.Io.net` bound-connect (`ConnectOptions.local_address`-class API) absorbs all four sites. |
