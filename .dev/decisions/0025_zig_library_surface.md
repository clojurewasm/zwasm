...
|------------|--------------|-----------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 2026-05-05 | `8a29f978` | initial   | Adopted; designs the Zig host-library surface explicitly (3-line happy path, fused Runtime, Module/Instance separation, dual-layer call API, WASI-as-option).                                                                                                                                                                                                          |
| 2026-05-05 | `8a29f978` | gap       | Self-review amendments (8 issues): D-1 zone placement clarified (facade in src/zwasm.zig, lib zone); D-3 "zero overhead" → "constant overhead" (corrected box/unbox cost); D-4 WASI prereq (wasi/host.zig WasiStdio union add) acknowledged; D-5 cross-module source `*Module` → `*Instance` (ADR-0014 §6.K.3 needs live runtime); D-7 ParseError+InstantiateError added to stable list; D-8 rationale updated (back-ref pattern + dispatch doesn't allocate); B-3 commit estimate +1; B-4 ImportBinding prereq stated; Phase C/D ordered as D before C. |
| 2026-05-11 | `3d0e8a7c` | gap       | Status flipped to `Accepted (partial — see D-075; design retained, implementation deferred to v0.1.0 RC)` per the 2026-05-11 ADR audit (`private/20250511_adr_audit/SUMMARY.md` §2.2). Implementation phases B-1..B-5 remained un-started through Phase 7 / 8 / 9; the ADR is **design**, not driven scope. The audit found `src/zwasm.zig` exposes the internal module-graph names but none of the curated facade `Runtime` / `Module` / `Instance` / `TypedFunc` / `WasiConfig` / `ImportEntry` top-level constants. D-075 carries the structural barrier ("no Phase row currently scheduled for the facade"). When v0.1.0 RC opens, the facade work re-claims a phase row and the partial qualifier comes off. ADR-0050 D-1's `Accepted (partial — see D-NNN)` notation is the matching Status form. |
