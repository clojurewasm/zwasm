...
| Date | SHA | Note |
|---|---|---|
| 2026-05-05 | `618ac144` | M3 phasing amended by ADR-0028: M3 is no longer "deferred to Phase 7 close". Phased as M3-a (bounds + trap, before §9.7 / 7.8 spec gate) → M3-b (regalloc + jit, before §9.7 / 7.11 differential) → M3-c (regir + exec, Phase 8+). M3-a-1 (ringbuffer infrastructure + compile-time bounds writes via `src/diagnostic/trace.zig` + `-Dtrace-ringbuffer` build flag) landed in this commit. |
| 2026-05-11 | `e85bd561` | **Path migration + golden-test verification gap** (per 2026-05-11 ADR audit, SUMMARY §3.3 + §5.2 / batch_B). Three honest records: (1) Decision § references `src/runtime/diagnostic.zig` (Zone 1); ADR-0023 §7 step 15 relocated this to its own `src/diagnostic/` zone — current path is `src/diagnostic/diagnostic.zig`. `Trap` enum reference `src/interp/mod.zig:137` is now `src/runtime/runtime.zig`'s domain (per ADR-0023). (2) `src/main.zig` references in §"Implementation guide / Phase 1" point at the post-ADR-0024 D-4 `src/cli/main.zig`. (3) The "Phase 1 deliverable: `test/cli/diag_print_test.zig` (golden)" was not landed at that path; the M1 golden coverage exists as a `test {}` block embedded in `src/cli/diag_print.zig` instead. The behavioural intent (golden CLI test for diag rendering) is satisfied; only the file location differs from the design statement. No load-bearing change; honest record only. |
| 2026-05-29 | `d8daef9b` | **M3 validate-location landed** (10.G). `Location.validate: { fn_idx, body_offset, opcode }` implemented (was the deferred M3 design). `validator.run`'s dispatch loop sets op/offset on the cold path; `frontendValidate` patches `fn_idx` (`noteValidateFuncIdx`) + sets phase-level diags for pre-func-loop failures (type-section decode / subtype validation). Rendered by the wasm-3.0 spec runner (`compile FAIL: <err> — <msg> [fn= off= op=]`) + `cli/run.zig` (no longer clobbers the validate diag). This is the permanent replacement for the throwaway op-probe used during GC bring-up (lesson `gc-type-subtyping-is-rtt-blocked`); validate failures now self-attribute. M3-b/M3-c (jit/exec locations) still pending. |
