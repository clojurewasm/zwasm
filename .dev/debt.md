...
| D-032 | code   | blocked-by: §9.7 / 7.7-fp-end-fix sub-chunk (FP-aware end handler) | x86_64 `emit.zig` function-level `end` handler emits `MOV EAX, r32(src)` for every result type, which truncates FP results (the value lives in an XMM, not the GPR `slotToReg(slot_id)` returns). Per ARM64's reference at `arm64/emit.zig:475-503`, FP returns should branch on `result_kind` and emit MOVAPS XMM0, src_xmm (or MOVSS / MOVSD) for f32 / f64. The current shape silently produces wrong return bytes when an FP value sits at the function's top-of-stack at `end`. **Surfaced 2026-05-06 by 7.7-fp-const landing**: byte-level emit tests for `f32.const` / `f64.const` pass (the const-handler region is asserted correctly), but compiled functions returning f32/f64 cannot be executed correctly until this is fixed. Same shape as the i64-result truncation gap noted in D-031. **Discharge** = ~20 LOC end-handler extension + 1 new encoder (encMovapsXmmXmm or MOVSS/MOVSD scalar moves) + a couple of tests. Held for the dedicated sub-chunk so this commit's diff stays focused on const handlers. | 2026-05-06 | 2026-05-06 | `src/engine/codegen/x86_64/emit.zig:end-handler-block`, ARM64 reference `src/engine/codegen/arm64/emit.zig:475-503`, ADR-0017 sub-2d-ii |
