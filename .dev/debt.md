...
| D-008 | infra  | now                                                    | **Updated 2026-05-04 after extended_challenge Step 1 walk**: `ssh windowsmini "wasmtime --version"` returned `wasmtime 42.0.1 (6844a83b5 2026-02-25)` — wasmtime is **NOT** a stub on windowsmini; it's real and runs. The diff_runner reports `skipped-wasmtime-fail` for every fixture which means `std.process.run(.{ argv = {wasmtime_path, "run", fixture_path} })` fails to spawn even though direct invocation works. Likely root cause: `which wasmtime` on windowsmini's bash shell returns a Unix-style path (`/c/...`) that Zig's child-process spawn on Windows interprets incorrectly, OR the default SSH shell is PowerShell which doesn't expose the bash-installed wasmtime to Zig's spawn. The previous "stub" framing was wrong — flip to `now` and either (a) make diff_runner resolve to a Windows-canonical path before spawn, OR (b) explicitly skip Windows from the diff gate via build option (Mac+Linux suffice for the differential gate). | 2026-05-04 | 2026-05-04 | `test/realworld/diff_runner.zig:171-183`, audit anchor `ssh windowsmini "wasmtime --version"` |
