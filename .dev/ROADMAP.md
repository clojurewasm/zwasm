...
| 12.1    | `.cwasm` loader — `zwasm run *.cwasm` reads a §9.8b-produced artefact against `format.zig`'s `CwasmHeader` + 12-byte `CwasmFuncMeta` + 9-byte `CwasmReloc` (ADR-0039 Rev 2; header v0.2 = 68 B w/ exports section per ADR-0138); load + relocate + execute via a standalone runtime. Stateful `.cwasm` (memory/globals/imports) → D-250.            | [x]  |
