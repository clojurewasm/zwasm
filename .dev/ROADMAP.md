...
| 8.2  | **D-051 discharge** — `x86_64/emit.zig` prologue extraction per mirror of ADR-0021 (arm64 `prologue.zig` pattern). Draft ADR `0030_x86_64_prologue_split.md`; extract prologue/epilogue into `engine/codegen/x86_64/prologue.zig` with `body_start_offset` helper; migrate test sites that hardcode byte offsets to use the helper; verify spill-aware staging path remains green (BASELINE delta check). Brings `x86_64/emit.zig` under §A2 2000 LOC hard cap. | [ ]            |
| 8.3  | **Windowsmini bench Phase 8.0 disposition** — per Phase 7 close finding (gate doc §5b, Mac:Win ratio 3-12x with fib2 33min/fixture): either define a windowsmini-specific 3-5 hot-fixture subset for periodic local verification, OR wire SSH-from-Linux-runner CI integration so windowsmini bench runs out-of-band of the inline gate cadence. | [ ]            |
