...
| 14.3 | `.github/workflows/nightly.yml` — fuzz + spec-bump + proposal-watch (`.dev/proposal_watch.md` quarterly cadence). **BLOCKED-BY D-256**: fuzz + spec-bump infra does not exist (no `test/fuzz/`, no `test-fuzz` step, no spec-bump checker — §3-scope feature never built). A fuzz-less nightly = redundant with §14.1 pr.yml. Build the fuzz harness + spec-bump checker FIRST, then wire. | [ ]  |
