...
| 7.5  | spec test pass=fail=skip=0 via ARM64 JIT on Mac aarch64 host (drives every Wasm 1.0 + 2.0 op the interp covers). `skip=0` semantics per ADR-0029: `skip-impl` (implementation-gap + test-shape) のみカウント、`skip-adr-<id>` (proposal-skip ADR 経由の deliberate 除外) は別 tally。 | [x]            |
