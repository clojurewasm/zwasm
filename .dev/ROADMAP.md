...
| 9.5  | ARM64 emit (NEON): SIMD load/store + lane access + integer arithmetic. Splits into 9.5-a (encoder foundation `[x]`), 9.5-b-i/ii/iii (shape-tag pipeline + per-op handlers + dispatch `[x]`), 9.5-c-i/ii (Q-form v128 spill helpers + op_simd refactor `[x]`), 9.5-c-iii (i32x4.extract_lane via UMOV W ← V.S[lane] + i32x4.replace_lane via INS V.S[lane] ← W; encUmovWFromS + encInsSFromW encoders + handlers + dispatch arms `[x]`), 9.5-c-iv (remaining int-arith op shapes: i8x16 / i16x8 / i64x2 add/sub/mul). | [ ] (9.5-a/b/c-i/c-ii/c-iii `[x]`) |
