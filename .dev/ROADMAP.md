...
| 13.3    | `wasi.h` + `zwasm.h` ABI surface complete (the binding's WASI config + zwasm-specific extensions; headers in `include/`). **Partial** (`set_args`/`set_envs`/`inherit_stdio` `47298cd1`); remainder (`inherit_argv`/`env`, `preopen_dir`) BLOCKED on the ADR-0070 C-API io/process-provenance decision → §13.4 sequenced first (ADR-0142). | [ ]  |
