...
| 9.5  | ARM64 emit (NEON): SIMD load/store + lane access + integer arithmetic. Splits into 9.5-a (NEON encoder foundation `[x]`), 9.5-b-i (shape-tag predicate + populator `[x]`), 9.5-b-ii (compute() integration `[x]`), 9.5-b-iii (per-op emit handlers + dispatch `[x]`), 9.5-c-i (16-byte Q-form v128 spill helpers `qLoadSpilled` / `qDefSpilled` / `qStoreSpilled` in gpr.zig — mirror of fp* trio with 16-byte stride; 6 unit tests `[x]`), 9.5-c-ii (lift SPILL-EXEMPT degradation in op_simd.zig handlers using new q* helpers + extract/replace_lane handlers + remaining int-arith op shapes). | [ ] (9.5-a + 9.5-b-i/ii/iii + 9.5-c-i `[x]`) |
