...
| 15.4 | **SIMD: coverage (D-246 DONE) + perf ports (measure-first)**. ✅ **D-246 RESOLVED** (`078ffde5`→`1029e5b4`): arm64 JIT SIMD emit now at full coverage parity with x86_64 — 26 ops closed (dot + 12 extmul + 8 sat-arith + q15mulr + 4 extadd_pairwise), all clang-verified encoders + the missing lowering arms + JIT execution fixtures. Residual: **perf ports** v1 W43/W44/W45 + W54-class hoist + Phase-11 gap candidates (AVX/CPUID, MOVAPS peephole) — **measure-first** per ADR-0149/0150 lesson (the §11.3 profile already found SIMD competitive: 0/12 ops lag >3×, so likely thin → fold to §15.P if so). | [ ]  |
...
| 15.P | Phase 15 close — **parity-vs-v1 validation** (fixed combined-≥10% replaced per ADR-0149/0150: regalloc-axis §15.2+§15.3 measured ~0 headroom, v2 emit already efficient): no unexplained regression vs v1 main + §15.4 SIMD deltas on SIMD-heavy fixtures + record the already-efficient finding + opportunistic D-259 spillBytes cleanup if a net win. + 3-host reconcile (D-245-dependent) + widget 15 → DONE + Phase 16 inline expand. | [ ]  |
