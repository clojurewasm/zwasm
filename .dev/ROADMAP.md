...
| 15.P | Phase 15 close — **parity-vs-v1 validation** (fixed combined-≥10% replaced per ADR-0149/0150: regalloc-axis §15.2+§15.3 measured ~0 headroom, v2 emit already efficient): no unexplained regression vs v1 main + the **W45 loop-isolated measurement** (≥50M-iter v128-local-carrying loop / no-op-module baseline subtraction, per ADR-0151 — if v2's per-iteration v128-reload dominates a long loop, reconsider W45 loop-persistence) + record the already-efficient finding (§15.2/15.3/15.4 all measured) + opportunistic D-259 spillBytes cleanup if a net win. + 3-host reconcile (D-245-dependent) + widget 15 → DONE + Phase 16 inline expand. | [ ]  |
