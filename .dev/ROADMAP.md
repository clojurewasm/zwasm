...
| WASI testsuite                                  | upstream `prod/testsuite-base` prebuilt binaries at a pinned SHA                                    | **`.wasm` + manifests + preopen trees committed** under `test/component/wasip3_official/` + `scripts/vendor_wasip3_official.sh` pin + regen (ADR-0205 D3). The same treatment for `wasm32-wasip1` is DECIDED but NOT YET IN THE TREE — ADR-0208 D1 specifies the corpus path and its vendor script (`wasm-tools strip`ped, 96.5 MiB → 5.5 MiB); both land with the implementation, not with the decision, so neither is referenced here until they exist. Three guest toolchains to rebuild either way — the heavyweight-toolchain exception |
