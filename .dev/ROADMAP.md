...
| 9.5  | ARM64 emit (NEON): SIMD load/store + lane access + integer arithmetic. Splits into 9.5-a (NEON encoder foundation `inst_neon.zig`: LDR Q / STR Q / ORR.16B / DUP.4S / ADD.4S `[x]`), 9.5-b-i (`zir.isSimdZirOp` + `regalloc.populateShapeTags` `[x]`), 9.5-b-ii (`compute()` integration `[x]`), 9.5-b-iii (per-op emit handlers in `arm64/op_simd.zig`: emitV128Load + emitV128Store + emitI32x4Splat + emitI32x4Add + emit.zig dispatch arms; SPILL-EXEMPT non-spilling MVP per D-034 — 16-byte v128 spill-aware path defers to 9.5-c), 9.5-c (extract/replace_lane + remaining int arith op shapes + 16-byte v128 spill helpers). | [ ] (9.5-a + 9.5-b-i/ii/iii `[x]`) |
