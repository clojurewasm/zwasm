...
| 12.3b   | **Stateful `.cwasm` execution (ADR-0139; promoted from D-250)** — serialise module state (memory limits + data segments, globals + init, tables + elem segments, imports) into `.cwasm` v0.3 + reconstruct a real runtime from the artefact alone (AOT analogue of `setup.setupRuntime`). Lifts the `aot/run.zig` stateless guard so real (memory-using) modules run. **§12.4 prerequisite** (real fixtures trap on the stateless path). | [ ]  |
...
| 12.4    | Cold-start bench-delta: load + first-call vs JIT first-invocation **≥30%** improvement on ≥3 v1-class hyperfine fixtures (the ADR-0040-deferred §9.8b/8b.3 bench obligation; concrete threshold from `private/notes/p8-8b3-aot-survey.md`). **Blocked on §12.3b** (ADR-0139): real v1-class fixtures use memory/globals → trap on the stateless AOT path; measured once §12.3b lands. | [ ]  |
