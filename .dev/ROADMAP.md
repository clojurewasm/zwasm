...
### Phase 9 — Wasm 1.0 + 2.0 (incl. SIMD) 完備 on 3 hosts
...
| 9.12    | 🔒 **Phase 9 完備 substrate re-examination** ([`.dev/phase9_completion_substrate_audit.md`](../phase9_completion_substrate_audit.md)) — collaborative review per ADR-0062. The 47 d-093 sub-chunks landed Wasm 2.0 100% PASS by direct edits to `switch (ZirOp)` arms across lower/validator/emit-arm64/emit-x86_64; this is at odds with ROADMAP §4.5 (dispatch-table feature modules) and §4.6 (`-Dwasm=` / `-Denable=` build flags) which were left as un-populated scaffolding. The audit revisits §2 P13 + P14, §4.5, §4.6 and decides the substrate architecture (hypotheses A: dispatch-table completion; B: comptime-gated switch; C: per-op-file + inline-switch hybrid; D: other) **before** Phase 10 feature work opens. The loop's hard-gate detector surfaces this row on next resume after 9.9 closes.                                                                                                                                                                                                                                                                                                                                                                      | [ ]                                  |
| 9.12-pre | **ADR drafts + 3 Q3 spikes (autonomous prep for §9.12 collab gate)** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3. Drafts: ADR-0070 (libc dependency policy) / ADR-0071 (Phase 9 substrate audit resolution = Q2 P14 sharpening + Q3 採択 + Q4 boundary) / ADR-0072 (comment-as-invariant rule) / ADR-0073 (build-option DCE 全 layer 一貫 substrate) + ADR-0050 amend (skip-impl one-way ratchet) + ADR-0023 §4.5 amend (per-op file pattern 正式採用). Spikes: `q3-zig-inline-switch/` (581-tag `inline switch` の Zig 0.16 compile time + IR 膨張計測) + `q3-interp-dispatch-bench/` (中央 DispatchTable interp vs `.always_tail` cycle 差) + `q3-build-option-dce-poc/` (代表 op `i32.add` を C パターンで実装し 6 build option 組合せで symbol/size/test 確認). Exit: 6 ADR が `Status: Proposed` で land + 3 spike が measurement report 出力済み. | [ ]                                  |
| 9.12-A  | **Iteration-speed scaffolding compression + あきらめないための enforcement layer 構築** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3 + §7. Compression: ROADMAP Phase 0-8 narrative archive (-800-1000 LOC); `continue/SKILL.md` 圧縮 (-300 LOC); closed phase gate / 古い audit / closed spike archive. Enforcement (9 item): build-option DCE 強制 (`scripts/check_build_dce.sh` + `audit §H`); per-op file 完全性 (`dispatch_collector.zig` comptime check); skip-impl one-way ratchet (`scripts/check_skip_impl_ratchet.sh` + `skip_impl_history.yaml`); anti-fallback (`.claude/rules/no_fallback_on_failure.md` + grep script); spike lifecycle (rule + audit); chunk-close literal exit gate; Q3 C consistency audit (skill); progress tracker yaml; feature_level comptime verification. Existing 8 gates の整理 + 統合余地. Exit: cold-start 読み込み -40%; gate_commit 平均 -20%; 9 enforcement 全 pre-commit/pre-push hook 済み. | [ ]                                  |
| 9.12-B  | **Q3 C 採択完成 + build-option DCE 全 layer 拡張** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3. Per-op file 全 op 移行 (残り Wasm 1.0 placeholder 4 件 + Wasm 2.0 `multi_value.zig` + SIMD-128 relocate); 中央 collector + dispatcher (`dispatch_collector.zig` + 5 dispatcher を inline switch + collector consumption に書き直し); Build-option DCE 全 layer 拡張 (CLI args declarative form / C API `comptime @export` filter + `wasm.h` preprocessor gate / WASI syscall feature_level metadata). Exit: 6 build option 組合せ (`-Dwasm={v1_0,v2_0,v3_0}` × `-Dwasi={p1,p2}`) で `test-all` 全 green; `check_build_dce.sh` 0; per-op file 完全性 comptime check 通過. | [ ]                                  |
| 9.12-C  | **Q5 hygiene landings** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3 + §3.5. `.claude/rules/comment_as_invariant.md` (新設); `abi.zig` comptime disjointness check 拡張 (table/memory emit scratch named-constant array 化); D-133 sweep (arm64 op_table/op_memory hardcoded X10/X11/X12 を named-constant 経由に); `.claude/rules/edge_case_testing.md` stress axes 節追加; `audit §G` grep 強化; `.claude/rules/bug_fix_survey.md` 強化 + `/continue` Step 4 checklist inline; `.claude/rules/runtime_instance_layer.md` 新設 (Cat III code zone rule). Exit: D-133 closed; comment_as_invariant rule auto-load 動作; audit grep 検出 0 件. | [ ]                                  |
| 9.12-D  | **Q6 libc 依存境界** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3 + §3.6. ADR-0070 Accepted (necessary / replaceable / convenience 3 区分); `.claude/rules/libc_boundary.md` auto-load; ROADMAP §14 amendment (unconscious libc fanout 禁止); `scripts/check_libc_boundary.sh` + `audit §G.5` 拡張; Sample migration `std.c.{write,_exit,getenv,munmap}` → `std.posix.*` (~5-10 sites). Exit: `bash scripts/check_libc_boundary.sh` 0; 全 host で `test-all` green. | [ ]                                  |
| 9.12-E  | **★ Wasm 2.0 完備 100% (skip-impl 243 → 0 + 網羅テスト 4 系統 green)** — Phase 9 完備の主軸 exit per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3 + §2.2. SKIP-CROSS-MODULE-IMPORTS 100 (imports/elem/data/linking/table*/memory*/global) discharge via `hasUnbindableImports()` reject 条件緩和 + per-shape resolver; SKIP-NO-LINK-TYPECHECK 26 via `Instance.checkImportType()` + `applyAssertUnlinkable` callback; SKIP-VALIDATOR-GAP SIMD 50 (simd_lane lane-index range + simd_align alignment immediate range); exports non-invoke-action 1 (action dispatcher `get`/`set`); D-079 v128 cross-module imports (ii) via ADR-0052 §3 globals 拡張. 網羅テスト: spec (skip-impl == 0) + edge_cases + realworld (Wasm 2.0 範囲; emcc 系は Phase 11 deferred) + differential vs wasmtime. Exit: `spec_assert_runner_non_simd: N passed, 0 failed, 495 skipped (= 0 skip-impl + 495 skip-adr)` Mac+ubuntunote bit-identical; `simd_assert_runner: 13301 passed, 0 failed, 390 skipped (= 0 skip-impl + 390 skip-adr)` Mac+ubuntunote bit-identical; 4 testsuite 系統 green; ratchet 0 維持. | [ ]                                  |
| 9.12-F  | **Phase-9-eligible debt cohort** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3. D-094 (x86_64 multi-result indirect-result-buffer; D-140/D-148 chain で dissolve 確認 or discharge); D-090 (lower.zig type-stack walker); D-062 (arm64 v128 9th+ stack overflow); D-141 (file_size_check WARN; Q3 C 採択で大部分 dissolve, 残り個別 ADR); D-081 (emit.zig source split; Q3 C で dissolve 確認); D-055 (emit_test_*.zig migration). Exit: debt active rows < 15. | [ ]                                  |
| 9.12-G  | **Phase 10 prep substrate** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3. ZirOp Wasm 3.0 slot ↔ Wasm spec 番号対応表 `.dev/wasm_3_0_zirop_mapping.md` (collector machine-generate); `src/instruction/wasm_3_0/` の placeholder を Phase 10 features 全体 (GC/EH/tail-call/memory64/multi-memory/typed func refs) に拡張; `src/api/instance.zig` (1424 LOC) health + helper extraction + c_api Instance-path test 最小カバレッジ (D-139 前倒し); CLI `--invoke` mode 追加 (Phase 11 bench 前提); `include/wasm.h` 上流 diff チェック; `zone_check.sh --gate` 移行 (info → enforce); `.dev/architecture/zone_layout.md` 新設. Exit: Phase 10 features 全 ZirOp が `comptime` で `Error.UnsupportedOpForBuildLevel` reject; `zone_check --gate` 0; c_api 基本 path テスト land. | [ ]                                  |
| 9.12-H  | **Bench baseline (Mac-only Wasm 2.0 + wasmtime 比較)** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3. `scripts/run_bench.sh --compare=wasmtime` + `--capture-rss` 拡張; Mac aarch64 ReleaseSafe で 26 fixtures × hyperfine `--warmup 3 --runs 5`; `bench/results/history.yaml` に `runtime: zwasm` / `wasmtime` 別 row 追加; D-074 partial 解消 (wazero/wasmer/bun/node + `-Dwith-bench-compare` flag は Phase 11). Exit: history.yaml に "p9-close: Wasm-2.0 baseline (Mac aarch64)" row; zwasm vs wasmtime mean_ms ratio doc 化. | [ ]                                  |
| 9.12-I  | **ADR + lesson + private/ closure** per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3. D-149 discharge (ADR Phase-9 cohort SHA backfill 75 → 0); ADR Status canonical pass (~22-25 件 `Accepted` → `Closed (Phase X DONE)`); skip-ADR Status 文言整理 (skip_cross_module_register canonical, skip_cross_module_action close 化候補); Lesson Citing backfill; Lesson promotion 候補スキャン (3+ citations は ADR 化判断). Exit: `check_adr_history.sh --gate` 0; `check_lesson_citing.sh` 0; ADR `Accepted` 数 < 30. | [ ]                                  |
| 9.13-0  | **Cat IV — windowsmini reconcile sweep + cross-platform sweep (relocated from §9.9-IV per ADR-0049 + ADR-0056 + ADR-0065 2026-05-18 amends; scope 拡張 per [`phase9_completion_close_plan.md`](../phase9_completion_close_plan.md) §5.3)**. Items: D-084 Win64 v128 marshal residual; D-136 Win64 SEH bridge for `assert_trap` recovery (likely small C/asm shim alongside Zig); D-028 windowsmini SSH test-runner IPC flake re-evaluate; any Windows-platform-specific issues surfacing from §9.9-III cross-module / host-import code OR §9.12 substrate audit cleanup. Sequence: inventory blocking items; pick implementation path per item; land in batch (single chunk if mechanical, multi-chunk if SEH bridge alone is substantial). Exit: windowsmini full `test-all` green including `spec_assert_runner_non_simd`, bit-identical with Mac + ubuntunote. windowsmini per-chunk gate stays deferred per ADR-0049 until this row opens. Sub-chunks recorded in `phase_log/phase9.md`. Runs autonomously by the `/continue` loop AFTER §9.12 substrate audit hard-gate clears (= §9.12 [x]); §9.13 Phase-10 entry hard-gate requires this row [x] for the 3-host invariant to hold.                                                                                                                                                                                                                                                                                                                                                                                                                                                            | [ ]                                  |
