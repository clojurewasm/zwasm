...
| B156 | §9.12-E Chunk B Part 11 — attempted to flip `hasUnbindableImports` for `.global` + wire `applySpectestGlobalImports`. Reverted on test surface regressions: 25352 → 25353 passed (+1), 0 → 6 failed. Errors: `imports.36.wasm compile: InvalidGlobalIndex`, `global/global.0.wasm compile: StackTypeMismatch`, plus 4× `data-init/table-init: UnsupportedEntrySignature`. Discovery: B154's globals_offsets refactor populates shape correctly but compileWasm's validator path + `applyTableInit`/`applyActiveDataSegments` have additional assumptions about no global imports that surface only when modules actually reach those code paths. | (revert) |
| **B157** | §9.12-E Chunk B Part 12 — investigate the 6 B156-regression sites. Likely needs (a) validator-side `num_global_imports`-aware indexing in global.get/set lowering (`src/validate/validator.zig` + `src/ir/lower.zig`), (b) applyTableInit / applyActiveDataSegments to either ignore global-import modules or correctly skip the imports prefix. Survey first; pick one of the 6 sites + investigate root cause. | **NEXT** |
