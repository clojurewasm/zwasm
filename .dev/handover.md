...
| B157 | §9.12-E Chunk B Part 12 — survey of B156's 6 regression sites. Findings (survey at private/notes/p9_12-E-B157-validator-runtime-investigation.md): Errors 1+2 (InvalidGlobalIndex / StackTypeMismatch) share root: `validator_globals` at runner.zig:733-738 includes only defined globals, but wasm index space requires imports-prefix. Errors 3+4 (table-init / data-init UnsupportedEntrySignature) are likely separate timing/init-order issues. 2 orthogonal fix chunks recommended. | `<this commit>` |
| **B158** | §9.12-E Chunk B Part 13 — fix `validator_globals` build at `src/engine/runner.zig:733-738` to mirror `globals_offsets` shape (imports prefix + defined). Walk imports section for global imports + insert their `valtype` (and `mutable` from imp.payload.global) into validator_globals BEFORE defined entries. Behaviour-preserving on current corpus (zero global imports today); unblocks Errors 1+2 from B156 once hasUnbindableImports is flipped. | **NEXT** |
