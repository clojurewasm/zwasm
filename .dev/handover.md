...
- **Plan**: chunk 1 = parse (0x08 + uleb page_size_log2, validate ∈{0,16}) + data shape (MemoryEntry +
  MemoryInstance `page_size_log2`) + parse unit tests (red). chunk 2 = thread page_size into setup alloc + interp
  memory.size/grow (byte units) + interp test. chunk 3 = JIT page-count shift `>>page_size_log2` both arches +
  edge fixtures (`(memory 1 1 (pagesize 1))`: memory.size = byte count, grow-by-N, fine-grained load/store).
...
- **Phase 17 (v0.2) IN-PROGRESS** (ADR-0168); **17.1-atomics DONE+3-host-confirmed @9eb84833** (full op set,
  ...
  (survey done; LARGE ~30 sites); NEXT = chunk-1 parse+data-shape. D-299 deferred.
