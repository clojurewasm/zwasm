...
| B154 | §9.12-E Chunk B Part 9 — non-empty-fn path refactored to use `computeGlobalsLayout`. Both compileWasm paths now produce identical shape. runner.zig shrinks 2000 → 1982 LOC. Closes shape inconsistency from B153. Behaviour-preserving on current corpus. | `<this commit>` |
| **B155** | §9.12-E Chunk B Part 10 — flip `hasUnbindableImports`'s Path #2 to call `isSpectestNonFuncBindable` (B147 predicate). For recognized spectest non-func imports, return false (= bindable) so the module reaches compileWasm. Architecture is ready: compileWasm produces (imports + defined)-sized globals layout; spec runner's scratch_globals sizing via `compiled.globals_byte_size` already accommodates the larger buffer; applyDefinedGlobalsInit shifts indexing past the imports prefix. Wire spectest-catalog `init_bits` writes into `scratch_globals[0..N*8]` at .module setup. Net expected: SKIP-CROSS-MODULE-IMPORTS count drops for the global-import cohort (38 imports + 18 data + ... etc. distribution from B145). | **NEXT** |
