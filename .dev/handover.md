...
| B155 | §9.12-E Chunk B Part 10 — `applySpectestGlobalImports(allocator, wasm_bytes, globals_offsets, scratch_globals)` helper in `spec_assert_runner_base.zig`. Walks imports + filters spectest globals + writes catalog `init_bits` to scratch_globals at the imports-prefix offsets. Not yet wired (no live callers). Behaviour-preserving. | `<this commit>` |
| **B156** | §9.12-E Chunk B Part 11 — flip `hasUnbindableImports`'s Path #2 for `.global` imports (call `isSpectestNonFuncBindable`; if recognized, return false / bindable). Add `applySpectestGlobalImports` call from `nonSimdOnModuleLoaded` + `simdOnModuleLoaded` (before `applyDefinedGlobalsInit`). Keep .table / .memory rejection for now (those need scratch_funcptrs / growable_memory wiring — separate piece). Net expected: SKIP-CROSS-MODULE-IMPORTS drops for the global-import cohort. | **NEXT** |
