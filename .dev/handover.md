...
9.9-g-8 (`<pending-sha>`): ARM64 shr_s/shr_u (8 ops via
NEG-then-(U|S)SHL + explicit MOVZ+AND mask) + critical mask
fix to 9.9-g-7's shl handler. simd_bit_shift fully wired.
+60 PASS (10727→10787). D-069 discharged.

**Mac aarch64 simd_assert_runner totals after 9.9-g-8**:
**10787 PASS** / **4 FAIL** / 2127 SKIP (over 21 manifests).
...
**Next 9.9-g-9 candidates** (in priority order):
...
- **D-066 spike** (simd_lane f64x2_extract_lane mismatch) —
  bounded; needs runtime spike (lldb body dump compare against
  passing func 22 / 23 in simd_lane.0).
...
9.9-f-1..8 + 9.9-g-1..8 landed; 9.9-g-9 NEXT).
