...
| B144 | §9.12-E Chunk A scope re-verification. **Discovery**: 0 manifest `skip-impl` lines in wasm-2.0-assert/ — all 100 SKIP-CROSS-MODULE-IMPORTS sites are RUNTIME emissions from `hasUnbindableImports` returning true in the `.module` dispatch arm (test/spec/spec_assert_runner_base.zig:2442-2446). Path #1 (`func` import from non-spectest non-registered) is already correctly handled — `hasUnbindableImports` allows registered modules through. Path #2 (`.table, .memory, .global => return true` unconditional, including for spectest!) is the actual gap: **most of the 100 SKIPs are `(import "spectest" "global_i32" (global i32))` shapes**, not registered-module cross-imports. Chunk A (func dispatch) is effectively already done; the real work is **Chunk B** = spectest non-func binding (global_i32 / global_i64 / global_f32 / global_f64 / table / memory). | (investigation-only) |
| **B145** | §9.12-E Chunk B Part 1 — spectest global binding. Add spectest.global_i32 / global_i64 / global_f32 / global_f64 bindings (Wasm spec testsuite values: i32=666, i64=666, f32=666.6, f64=666.6). Wire as a static catalog + extend hasUnbindableImports to recognize these (kind=global, valtype matching). Reuse `engine.export_lookup` patterns for read access at instantiation time. Est. ~80-120 LOC. Likely drops SKIP-CROSS-MODULE-IMPORTS by 10-15 sites. | **NEXT** |
