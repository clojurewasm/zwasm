...
1. **D-034 SIMD spill arc CLOSED @411dd1e14** — no active bundle; ZERO `now`-class debt; **D-460 v128-GC arc ALSO
   ...
   完成形 plateau: remaining partials are parked (D-330/331), consumer-gated (D-305), or conformance-neutral polish
   (D-294-R2 code-2 CLI sub-split — low value, suites match by kind). **NEXT (autonomous, your choice): (a) the
   D-034 helper-consolidation** (dedup resolveOrLoadV128/loadV128Into*/dstHomeOrXmm7*/store* across op_simd_float/
   int_cmp_lane/int_arith into a shared op_simd.zig pub set — behaviour-preserving, byte-asserting unit tests +
   spill fixtures are the net; ideally after windows confirms the spill arc), **or (b) an opportunistic
